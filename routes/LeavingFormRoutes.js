...
router.post('/submit-leaving-form', verifyToken, async (req, res) => {
    try {
        const studentId = req.user.studentId;
        const { studentName, parentMobile, reason, leavingTime, arrivalTime } = req.body;

        // Check if all fields are provided
        if (!studentName || !parentMobile || !reason || !leavingTime || !arrivalTime) {
